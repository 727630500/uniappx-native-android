...
open class GenComponentsQuestionOptionsTestQuestionOptionsTest : VueComponent {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {}
    open var options: UTSArray<Options> by `$props`
    open var resultOptions: UTSArray<Options> by `$props`
    open var userOption: String by `$props`
    open var modelValue: Number by `$props`
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenComponentsQuestionOptionsTestQuestionOptionsTest) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenComponentsQuestionOptionsTestQuestionOptionsTest
            ...
            fun gen_highlightText_fn(text: String, keyword: String?): UTSArray<underline1>? {
                if (keyword == null) {
                    return _uA<underline1>(underline1(text = text, highlight = false))
                }
                if (keyword == "" || text == "") {
                    return null
                }
                ...
                val result = _uA<underline1>()
                run {
                    var i: Number = 0
                    while(i < parts.length){
                        val part = parts[i]
                        if (part == keyword) {
                            result.push(underline1(text = part, highlight = true))
                        } else if (part !== "") {
                            result.push(underline1(text = part, highlight = false))
                        }
                        i++
                    }
                }
                        ...
                        _cE(Fragment, null, RenderHelpers.renderList(props.options, fun(option, index, __index, _cached): Any {
                            return _cE("view", _uM("key" to index, "class" to _nC(_uA(
                                ...
                                _cE("view", null, _uA(
                                    ...
                                    if (isTrue(option.picUrlList != null && option.picUrlList!!.length > 0)) {
                                        _cE("view", _uM("key" to 0), _uA(
                                            _cE(Fragment, null, RenderHelpers.renderList(option.picUrlList, fun(src, __key, __index, _cached): Any {
