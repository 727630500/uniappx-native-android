...
open class GenPagesSentenceDictationTest : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesSentenceDictationTest) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesSentenceDictationTest
            val _cache = __ins.renderCache
            val sendUpClassFn = sendUpClass()
            val studyTaskId = ref<Number?>(null)
            val isShowInput = ref<Boolean>(true)
            val _countDown = countDown()
            val answerMapNow = ref<topic4?>(null)
            val answerMap = Map<Number, topic4>()
            ...
            fun gen_showKeyboardFn_fn(index: Number, str: String, isShow: Boolean) {
                var _tempTiMuArr = answerMapNow.value?.tempShow
                if (_tempTiMuArr != null) {
                    var _str = str
                    var id = nowSub.value?.id
                    if (id == null) {
                        return
                    }
                    var isErr = false
                    var isAddTemp = false
                    var _tempTiMuArr = answerMapNow.value
                    if (_tempTiMuArr != null) {
                        _tempTiMuArr.tempShow!![index].content = _str
                        isErr = _tempTiMuArr.tempShow?.find(fun(item: tempTiMu): Boolean {
                            return item.title != item.content
                        }
                        ) != null
                        isAddTemp = _tempTiMuArr.tempShow?.find(fun(item: tempTiMu): Boolean {
                            return item.content != ""
                        }
                        ) != null
                        answerMap.set(id, _tempTiMuArr)
                    }
                    answerMapNow.value = _tempTiMuArr
                    if (isAddTemp) {
                        addTemp()
                    }
                    if (isErr) {
                        if (successList.has(id)) {
                            successList.`delete`(id)
                        }
                        failList.add(id)
                    } else {
                        if (failList.has(id)) {
                    ...
                    sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = nowSub.value!!.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子听写"), problemId = nowSub.value!!.id, englishText = nowSub.value!!.englishText, problemIndex = "0", isPass = if (isErr) {
                        "0"
                    } else {
                        "1"
                    }
                ...
                if (needShowKeyboard.value && isShow) {
            ...
            fun gen_submit_fn() {
                ...
                , "module" to getModelKey("同步句子"), "subModule" to getSubModelKey("同步句子听写"), "score" to scoreVal.value, "startTime" to startTime.value, "passStatus" to passStatus, "second" to secondVal.value, "studyTaskId" to studyTaskId.value, "type" to if (studyTaskId.value != null) {
                    "10"
                } else {
                    "0"
                }
            ...
            watch(_nowTitle, fun(kVal: String, oldVal: String){
                var id = nowSub.value?.id
                if (id == null) {
                    return
                }
                ...
                var _tempTiMuArr = answerMapNow.value
                if (_tempTiMuArr != null) {
                    if (kVal.length > _tempTiMuArr.tempShow!![inputIndex.value].title.length) {
                        _nowTitle.value = oldVal
                    }
                    _tempTiMuArr.tempShow!![inputIndex.value].content = kVal
                    isErr = _tempTiMuArr.tempShow?.find(fun(item: tempTiMu): Boolean {
                        return item.title != item.content
                    }
                    ) != null
            ...
            watch(isShowKeyboard, fun(kVal: Boolean){
                if (kVal) {
                    ...
                    styleTransformWithOrigin.value?.style?.setProperty("transform", "translateY(-70px)")
                } else {
                    styleTransformWithOrigin.value?.style?.setProperty("transform", "translateY(0)")
                }
            }
            )
            fun gen_makeInitTemp_fn(kVal: followAlongItem): topic4 {
                ...
                return topic4(item = kVal, tempShow = JSON.parse<UTSArray<tempTiMu>>(JSON.stringify(_tempTiMuArr)))
            }
            ...
            fun gen_getSubjectListTask_fn() {
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/studyTask/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子听写"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = "", options = item.options ?: _uA()))
                        }
                        )
                        startCountdown()
            ...
            fun gen_getReviewList_fn() {
                ...
                if (uni_getStorageSync("reviewList") != "") {
                    ...
                    if (_tempData != null) {
                        ...
                        answerMapNow.value = answerMap.get(_tempData[0].id)
                        tempShow.value = answerMapNow.value?.tempShow
                        startTime.value = formatDate(null, "YYYY-MM-DD HH:mm:ss")
                ...
                uni_request<Result<UTSJSONObject>>(RequestOptions(url = getUrl("/biz/problem/api/getExerciseRecordDetail"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.getArray("appProblemList")!!.forEach(fun(item, index){
                            ...
                            answerMap.set(__item.id, topic4(item = __item, tempShow = JSON.parse<UTSArray<tempTiMu>>(JSON.stringify((item as UTSJSONObject).getArray("tempShow")!!))))
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = (item as UTSJSONObject).getNumber("textbookUnitId") ?: 0, studyTaskId = 0, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子听写"), problemId = (item as UTSJSONObject).getNumber("id") ?: 0, englishText = (item as UTSJSONObject).getString("englishText") ?: "0", problemIndex = index.toString(10), isPass = (item as UTSJSONObject).getString("rightStatus") ?: "0", isExercise = "1", didDo = true, selectOpt = ""))
                        }
                        )
                        ...
                        _cV(_component_navbar, _uM("showBack" to true, "title" to ""), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                            return _uA(
                                _cV(_component_BackgroundImage, _uM("src" to "/static/ico/follow.png", "bgStyle" to "width:100%;height:100%;position: absolute;left:0;top:0", "className" to "title_box"), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                    return _uA(
                                        _cE("text", _uM("class" to "_text"), " 句子智能听写学后测试 ")
                                    )
                                }
                                ), "_" to 1))
                            )
                            ...
                            _cE("view", _uM("class" to "right_content"), _uA(
                                if (isTrue(!unref(isShowResult))) {
                                    _cE("view", _uM("key" to 0, "style" to _nS(_uM("flex" to "1"))), _uA(
                                        _cE("view", _uM("id" to "styleTransformWithOrigin", "style" to _nS(_uM("flex" to "1"))), _uA(
                                            ...
                                            _cE("view", _uM("class" to "word_box", "style" to _nS(_uM("align-items" to "center"))), _uA(
                                                _cE("view", null, _uA(
                                                    _cV(_component_u_playMp3, _uM("src" to unref(answerMapNow)?.item?.soundUrl, "isAuto" to !unref(isShowResult), "tplType" to 4), null, 8, _uA(
                                                        "src",
                                                        "isAuto"
                                                    ))
                                                )),
                                                _cE("view", _uM("style" to _nS(_uM("margin-top" to "8rpx"))), _uA(
                                                    _cV(_component_memoryLine, _uM("num" to unref(answerMapNow)?.item?.currentMemoryScore), null, 8, _uA(
                                                        "num"
                                                    ))
                                                ), 4),
                                                _cE("view", _uM("style" to _nS(_uM("margin" to "20rpx 0", "padding" to "0 22rpx", "width" to "100%"))), _uA(
                                                    if (isTrue(unref(isShowInput))) {
                                                        _cV(_component_subCheckListInput, _uM("key" to 0, "list" to unref(answerMapNow)?.tempShow, "onShowKeyboard" to showKeyboardFn), null, 8, _uA(
                                                            "list"
                                                        ))
                                                    } else {
                                                        _cC("v-if", true)
                                    ...
                                    _cE("view", _uM("key" to 1, "class" to "adopt_box", "style" to _nS(_uM("flex" to "1"))), _uA(
                                        _cE("view", _uM("class" to "top_box", "style" to _nS(_uM("justify-content" to "flex-end", "align-items" to "center"))), _uA(
                                            ...
                                            ,
                                            _cE("view", _uM("style" to _nS(_uM("flex" to "1", "align-items" to "center", "justify-content" to "center"))), _uA(
                                                _cE("view", null, _uA(
                                                    _cV(_component_u_playMp3, _uM("src" to unref(answerMapNow)?.item?.soundUrl, "tplType" to 4), null, 8, _uA(
                                                        "src"
                                                    ))
                                                )),
                                                _cE("view", _uM("style" to _nS(_uM("margin-top" to "8rpx"))), _uA(
                                                ...
                                                _cE("text", _uM("class" to "word_text", "style" to _nS(_uM("margin-top" to "10rpx"))), _tD(unref(answerMapNow)?.item?.englishText), 5)
                                            ), 4),
                                            _cV(_component_contrastSentence, _uM("arr" to unref(answerMapNow)?.tempShow), null, 8, _uA(
                                                "arr"
                                            ))
                                        ), 4),
                                        _cE("view", _uM("style" to _nS(_uM("padding" to "0 21rpx 10rpx", "flex" to "1", "align-items" to "center", "justify-content" to "center"))), _uA(
                                            _cE("view", _uM("style" to _nS(_uM("justify-content" to "flex-end", "align-items" to "center", "flex" to "1"))), _uA(
                                                _cV(_component_adoptResult, _uM("score" to unref(scoreVal), "notDone" to unref(notDoneNum), "failNum" to unref(failList).size, "time" to unref(timeText)), null, 8, _uA(
                                                    "score",
                                                    "notDone",
                                                    "failNum",
                                                    "time"
