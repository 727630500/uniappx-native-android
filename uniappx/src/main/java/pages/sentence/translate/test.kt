...
open class GenPagesSentenceTranslateTest : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesSentenceTranslateTest) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesSentenceTranslateTest
            val _cache = __ins.renderCache
            val sendUpClassFn = sendUpClass()
            val studyTaskId = ref<Number?>(null)
            val _countDown = countDown()
            val answerMapNow = ref<topic2?>(null)
            val answerMap = Map<Number, topic2>()
            ...
            fun gen_submit_fn() {
                ...
                , "module" to getModelKey("同步句子"), "subModule" to getSubModelKey("同步句子翻译"), "score" to scoreVal.value, "startTime" to startTime.value, "passStatus" to passStatus, "second" to secondVal.value, "studyTaskId" to studyTaskId.value, "type" to if (studyTaskId.value != null) {
                    "10"
                } else {
                    "0"
                }
            ...
            fun gen_subClear_fn() {
                ...
                nowItem = JSON.parse<topic2?>(JSON.stringify(nowItem))
                if (nowItem == null) {
                    return
                }
            ...
            fun gen_clickItem_fn(kVal: tempTiMuParameter) {
                ...
                _item = JSON.parse<topic2?>(JSON.stringify(_item))
                if (_item == null) {
                    return
                }
                ...
                sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = _item.item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子翻译"), problemId = _item.item.id, englishText = _item.item.englishText, problemIndex = "0", isPass = if (isErrVal) {
                    "0"
                } else {
                    "1"
                }
            ...
            fun gen_makeInitTemp_fn(kVal: followAlongItem): topic2 {
                ...
                return topic2(item = kVal, tempCheck = JSON.parse<UTSArray<tempTiMu>>(JSON.stringify(_tempTiMuArr)), tempShow = _makeTemp)
            }
                ...
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/studyTask/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子翻译"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = "", options = item.options ?: _uA()))
                        }
                        )
                        startCountdown()
            ...
            fun gen_getSubjectList_fn() {
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    var subModule = getSubModelKey("同步句子翻译")
                    var isExercise = "1"
            ...
            fun gen_getReviewList_fn() {
                ...
                if (uni_getStorageSync("reviewList") != "") {
                    ...
                    if (_data != null && _data.length > 0) {
                        ...
                        tempCheck.value = answerMapNow.value?.tempCheck
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子翻译"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = ""))
                        }
                        )
                        startCountdown()
                        startTime.value = formatDate(null, "YYYY-MM-DD HH:mm:ss")
                ...
                uni_request<Result<UTSJSONObject>>(RequestOptions(url = getUrl("/biz/problem/api/getExerciseRecordDetail"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.getArray("appProblemList")!!.forEach(fun(item, index){
                            ...
                            answerMap.set(__item.id, topic2(item = __item, tempShow = JSON.parse<UTSArray<tempTiMu>>(JSON.stringify((item as UTSJSONObject).getArray("tempShow")!!))))
                        ...
                        _cV(_component_navbar, _uM("showBack" to true, "title" to ""), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                            return _uA(
                                _cV(_component_BackgroundImage, _uM("src" to "/static/ico/follow.png", "bgStyle" to "width:100%;height:100%;position: absolute;left:0;top:0", "className" to "title_box"), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                    return _uA(
                                        _cE("text", _uM("class" to "_text"), " 单元重点句翻译学后测试 ")
                                    )
                                }
                                ), "_" to 1))
                            )
