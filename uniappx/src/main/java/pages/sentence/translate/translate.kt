...
open class GenPagesSentenceTranslateTranslate : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesSentenceTranslateTranslate) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesSentenceTranslateTranslate
            ...
            _SequentialIterator.subModule = getSubModelKey("同步句子翻译")
            ...
            fun gen_sendUp_fn(isPass: String) {
                ...
                var _data: UTSJSONObject = _uO("textbookId" to getTextBookId(), "textbookUnitId" to nowItem.value?.textbookUnitId, "module" to getModelKey("同步句子"), "subModule" to getSubModelKey("同步句子翻译"), "problemId" to nowItem.value?.id, "englishText" to nowItem.value?.englishText, "problemIndex" to _SequentialIterator.getIndex(nowItem.value), "isPass" to isPass, "isExercise" to "0", "studyTaskId" to studyTaskId.value)
            ...
            fun gen_goTest_fn() {
                ...
                if (studyTaskId.value != 0) {
                    uni_redirectTo(RedirectToOptions(url = "/pages/sentence/translate/test?studyTaskId=" + studyTaskId.value))
                } else {
                    uni_redirectTo(RedirectToOptions(url = "/pages/sentence/translate/test"))
                }
            }
                                ...
                                _cE("view", _uM("class" to "top_box"), _uA(
                                    _cE("view", _uM("class" to "uni-row", "style" to _nS(_uM("align-items" to "center"))), _uA(
                                        ...
                                        if (isTrue(unref(_SequentialIterator).is80 || unref(_SequentialIterator).isErr80)) {
                                            _cE("text", _uM("key" to 0, "style" to _nS(_uM("color" to "#939393", "margin-left" to "6rpx"))), "复习", 4)
                                        } else {
                                            _cC("v-if", true)
                                        }
                                    ), 4),
                                ...
                                if (isTrue(unref(isShowResult))) {
                                    _cE("view", _uM("key" to 1, "class" to "content_box", "style" to _nS(_uM("align-items" to "center"))), _uA(
                                        if (isTrue(unref(isFail))) {
                                            ...
                                            _cV(_component_wj_karaoke, _uM("key" to 1, "id" to "enKaraoke", "forcePlay" to true, "isBold" to true, "fontSize" to 60, "songArr" to unref(songArr), "maxWidth" to unref(leftWidth), "center" to "center", "paly" to true, "style" to _nS(_uM("width" to "100%", "height" to "80rpx"))), null, 8, _uA(
                                                "songArr",
                                                "maxWidth",
                                                "style"
                                            ))
