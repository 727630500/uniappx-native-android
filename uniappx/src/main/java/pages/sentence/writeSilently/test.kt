...
open class GenPagesSentenceWriteSilentlyTest : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesSentenceWriteSilentlyTest) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesSentenceWriteSilentlyTest
            ...
            val answerMapNow = ref<topic3?>(null)
            val answerMap = Map<Number, topic3>()
            ...
            fun gen_showKeyboardFn_fn(index: Number, str: String, isShow: Boolean) {
                ...
                if (_tempTiMuArr != null) {
                    ...
                    sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = nowSub.value!!.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子默写"), problemId = nowSub.value!!.id, englishText = nowSub.value!!.englishText, problemIndex = "0", isPass = if (isErr) {
                        "0"
                    } else {
                        "1"
                    }
            ...
            fun gen_submit_fn() {
                ...
                , "module" to getModelKey("同步句子"), "subModule" to getSubModelKey("同步句子默写"), "score" to scoreVal.value, "startTime" to startTime.value, "passStatus" to passStatus, "second" to secondVal.value, "studyTaskId" to studyTaskId.value, "type" to if (studyTaskId.value != null) {
                    "10"
                } else {
                    "0"
                }
            ...
            fun gen_makeInitTemp_fn(kVal: followAlongItem): topic3 {
                ...
                return topic3(item = kVal, tempShow = JSON.parse<UTSArray<tempTiMu>>(JSON.stringify(_tempTiMuArr)))
            }
            ...
            fun gen_getSubjectListTask_fn() {
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/studyTask/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子默写"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = "", options = item.options ?: _uA()))
                        }
                        )
                        startCountdown()
            ...
            fun gen_getSubjectList_fn() {
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    var subModule = getSubModelKey("同步句子默写")
                    var isExercise = "1"
            ...
            fun gen_getReviewList_fn() {
                ...
                if (uni_getStorageSync("reviewList") != "") {
                    ...
                    if (_data != null && _data.length > 0) {
                        ...
                        startTime.value = formatDate(null, "YYYY-MM-DD HH:mm:ss")
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子默写"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = ""))
                        }
                        )
                        startCountdown()
                        timer.value = setInterval(fun(){
                ...
                uni_request<Result<UTSJSONObject>>(RequestOptions(url = getUrl("/biz/problem/api/getExerciseRecordDetail"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.getArray("appProblemList")!!.forEach(fun(item, index){
                            ...
                            answerMap.set(__item.id, topic3(item = __item, tempShow = JSON.parse<UTSArray<tempTiMu>>(JSON.stringify((item as UTSJSONObject).getArray("tempShow")!!))))
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = (item as UTSJSONObject).getNumber("textbookUnitId") ?: 0, studyTaskId = 0, module = getModelKey("同步句子"), subModule = getSubModelKey("同步句子默写"), problemId = (item as UTSJSONObject).getNumber("id") ?: 0, englishText = (item as UTSJSONObject).getString("englishText") ?: "0", problemIndex = index.toString(10), isPass = (item as UTSJSONObject).getString("rightStatus") ?: "0", isExercise = "1", didDo = true, selectOpt = ""))
                        }
                        )
                        subList.value = _arrData
                        nowSub.value = _arrData[0]
                        var _item = answerMap.get(_arrData[0].id)
                        console.log(_item, _arrData[0].id)
                        answerMapNow.value = _item
                        tempShow.value = answerMapNow.value?.tempShow
                        ...
                        _cV(_component_navbar, _uM("showBack" to true, "title" to ""), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                            return _uA(
                                _cV(_component_BackgroundImage, _uM("src" to "/static/ico/follow.png", "bgStyle" to "width:100%;height:100%;position: absolute;left:0;top:0", "className" to "title_box"), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                    return _uA(
                                        _cE("text", _uM("class" to "_text"), " 句子智能默写学后测试 ")
                                    )
                                }
                                ), "_" to 1))
                            )
                                    ...
                                    _cE("view", _uM("key" to 0, "style" to _nS(_uM("flex" to "1"))), _uA(
                                        _cE("view", _uM("id" to "styleTransformWithOrigin", "style" to _nS(_uM("flex" to "1"))), _uA(
                                            ...
                                            _cE("view", _uM("class" to "word_box", "style" to _nS(_uM("align-items" to "center"))), _uA(
                                                _cE("text", _uM("style" to _nS(_uM("font-size" to "28rpx"))), _tD(unref(answerMapNow)?.item?.chineseExplain), 5),
