...
open class GenPagesSentenceWriteSilentlyWriteSilently : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesSentenceWriteSilentlyWriteSilently) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesSentenceWriteSilentlyWriteSilently
            ...
            _SequentialIterator.subModule = getSubModelKey("同步句子默写")
            ...
            fun gen_sendUp_fn(isPass: String) {
                ...
                var _data: UTSJSONObject = _uO("textbookId" to getTextBookId(), "textbookUnitId" to nowItem.value?.textbookUnitId, "module" to getModelKey("同步句子"), "subModule" to getSubModelKey("同步句子默写"), "problemId" to nowItem.value?.id, "englishText" to nowItem.value?.englishText, "problemIndex" to smailNum.value, "isPass" to isPass, "isExercise" to "0", "studyTaskId" to studyTaskId.value)
                                ...
                                if (isTrue(!unref(isShowResult))) {
                                    _cE(Fragment, _uM("key" to 0), _uA(
                                        _cE("view", _uM("class" to "content_box"), _uA(
                                            _cE("text", _uM("style" to _nS(_uM("font-size" to "35rpx"))), _tD(unref(nowItem)?.chineseExplain), 5)
                                        )),
                                ...
                                if (isTrue(unref(isShowResult))) {
                                    _cE("view", _uM("key" to 1, "class" to "content_box", "style" to _nS(_uM("align-items" to "center"))), _uA(
                                        if (isTrue(unref(isFail))) {
                                            ...
                                            _cV(_component_wj_karaoke, _uM("key" to 1, "isBold" to true, "forcePlay" to true, "fontSize" to 60, "songArr" to unref(songArr), "maxWidth" to unref(leftWidth), "center" to "center", "paly" to true, "style" to _nS(_uM("width" to "100%", "height" to "80rpx"))), null, 8, _uA(
                                                "songArr",
                                                "maxWidth",
                                                "style"
                                            ))
