...
open class GenPagesTrainingRecordList : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesTrainingRecordList) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesTrainingRecordList
            ...
            fun gen_startStudy_fn(item: wordListItem) {
                var module = item.textbookUnitModule
                var subModule = item.textbookUnitSubModule
                var path = ""
                if (module == "100" && subModule == "109") {
                    path = "/pages/wordInter/learning/test?type=40&isTask=1&recordId=" + item.id + "&isTraining=1"
                } else if (module == "500" && subModule == "505") {
                    path = "/pages/examRecords/examRecords?resultId=" + item.id + "&title=模式二：语法训练" + "&isYF=1"
                } else if (module == "400" && subModule == "402") {
                    path = "/pages/examRecords/examRecords?resultId=" + item.id + "&title=模式三：阅读训练"
                }
                if (path == "") {
                    return
                }
                                ...
                                _cE("view", _uM("class" to "word_table"), _uA(
                                    _cE("view", _uM("class" to "word_table-header"), _uA(
                                        ...
                                        _cE("view", _uM("class" to "word_table-header-item word_table-header-item-unit-box"), _uA(
                                            _cE("text", _uM("class" to "word_table-header-item-text"), "备注")
                                        )),
                                        _cE("view", _uM("class" to "word_table-header-item word_table-header-item-status-box"), _uA(
                                        ...
                                        _cE(Fragment, null, RenderHelpers.renderList(unref(wordList), fun(item, index, __index, _cached): Any {
                                            return _cE("view", _uM("class" to "word_table-body-row", "key" to item, "style" to _nS("" + (if (((index + 1) % 2) == 0) {
                                                ...
                                                _cE("view", _uM("class" to "word_table-body-item word_table-body-item-unit-box"), _uA(
                                                    _cE("text", _uM("class" to "word_table-body-item-text"), _tD(item.sysRemark), 1)
                                                )),
