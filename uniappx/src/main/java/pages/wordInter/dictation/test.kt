...
open class GenPagesWordInterDictationTest : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesWordInterDictationTest) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesWordInterDictationTest
            ...
            fun gen_submit_fn() {
                ...
                , "module" to getModelKey("同步单词"), "subModule" to getSubModelKey("单词智能听写"), "score" to scoreVal.value, "startTime" to startTime.value, "passStatus" to passStatus, "second" to secondVal.value, "studyTaskId" to studyTaskId.value, "type" to if (studyTaskId.value != null) {
                    "10"
                } else {
                    "0"
                }
            ...
            fun gen_inspectText_fn() {
                ...
                if (_followAlongList != null) {
                    ...
                    sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = _now.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听写"), problemId = _now.id, englishText = _now.englishText, problemIndex = nowIndex.value.toString(10), isPass = if (inputText.value == _now.englishText) {
                        "1"
                    } else {
                        "0"
                    }
                ...
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/studyTask/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听写"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = "", options = item.options ?: _uA()))
                        }
                        )
                    }
                }
            ...
            fun gen_getSubjectList_fn() {
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        , 1000)
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听写"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = ""))
                        }
                        )
                    }
                }
            ...
            fun gen_test_fn() {
                ...
                if (_nowSub != null) {
                    ...
                    sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = _nowSub.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听写"), problemId = _nowSub.id, englishText = _nowSub.englishText, problemIndex = nowIndex.value.toString(10), isPass = "1", isExercise = "0", options = _nowSub.options ?: _uA()))
                }
                nextItem()
            }
                                    ...
                                    if (isTrue(!unref(isShowResult))) {
                                        _cE("view", _uM("key" to 0, "style" to _nS(_uM("flex" to "1"))), _uA(
                                            ...
                                            _cE("view", _uM("class" to "word_box", "style" to _nS(_uM("align-items" to "center", "margin-top" to "-30rpx"))), _uA(
                                                _cE("view", null, _uA(
                                                    _cV(_component_u_playMp3, _uM("src" to unref(nowSub)?.soundUrl, "tplType" to 4, "isAuto" to true, "ref" to "refMp3"), null, 8, _uA(
                                                        "src"
                                                    ))
                                                )),
                                                _cE("view", _uM("class" to "word_inp_box"), _uA(
                                                    _cE("view", _uM("style" to _nS(_uM("flex" to "1", "height" to "100%", "position" to "relative", "justify-content" to "center"))), _uA(
                                                        _cV(_component_wj_input, _uM("style" to _nS(_uM("width" to "100%", "height" to "32rpx")), "text" to unref(inputText), "onUpdate:text" to fun(`$event`: String){
                                    ...
                                    if (isTrue(unref(isShowResult))) {
                                        _cE("view", _uM("key" to 1, "class" to "adopt_box"), _uA(
                                            ...
                                            _cE("view", _uM("class" to "word_box"), _uA(
                                                _cE("view", null, _uA(
                                                    _cV(_component_u_playMp3, _uM("src" to unref(nowSub)?.soundUrl, "tplType" to 4, "ref" to "refMp3"), null, 8, _uA(
                                                        "src"
                                                    ))
                                                )),
                                                ...
                                                _cE("view", _uM("class" to "title", "style" to _nS(_uM("justify-content" to "flex-start"))), _uA(
                                                    ...
                                                    _cE("text", _uM("class" to "num", "style" to _nS(_uM("margin-left" to "20rpx"))), _tD(unref(nowSub)?.phoneticSymbol), 5)
                                                ), 4)
                                            ))
                                        ))
                                    } else {
