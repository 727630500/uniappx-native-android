...
open class GenPagesWordInterFollowAlongTest : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesWordInterFollowAlongTest) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesWordInterFollowAlongTest
            ...
            val nowExplainText = computed(fun(): String {
                var _subList = subList.value
                if (_subList == null) {
                    return ""
                }
                var _len = _subList.length
                return nowSub.value?.chineseExplain ?: ""
            }
            )
            val nowIndexSpeakUrl = computed(fun(): String {
                val arr = UTSArray.from(sendUpClassFn.get() as UTSArray<sendUpType>)
                val obj = arr[nowIndex.value] as sendUpType
                return obj.userSpeakUrl ?: ""
            }
            )
            val language = computed(fun(): String {
                var _subList = subList.value
                if (_subList == null) {
                    return ""
                }
                var _len = _subList.length
                return if (nowIndex.value <= _len / 2) {
                    "英语"
                } else {
                    "英语"
                }
            ...
            val nowExplainType = computed(fun(): String {
                var _subList = subList.value
                if (_subList == null) {
                    return ""
                }
            ...
            fun gen_submit_fn() {
                ...
                , "module" to getModelKey("同步单词"), "subModule" to getSubModelKey("单词智能听说"), "score" to scoreVal.value, "startTime" to startTime.value, "passStatus" to passStatus, "second" to secondVal.value, "studyTaskId" to studyTaskId.value, "type" to if (studyTaskId.value != null) {
                    "10"
                } else {
                    "0"
                }
            ...
            fun gen_test_fn() {
                ...
                if (_nowSub != null) {
                    if (!successList.value.includes(_nowSub.id)) {
                        ...
                        sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = _nowSub.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听说"), problemId = _nowSub.id, englishText = _nowSub.englishText, problemIndex = nowIndex.value.toString(10), isPass = "1", isExercise = "1", didDo = true, selectOpt = ""))
                    }
                }
                var _subList = subList.value
                if (_subList != null) {
            ...
            val recognitionSuccess = fun(data: recognitionSuccessType){
                ...
                if (nowSub.value != null) {
                    nowSub.value!!.totalScore = Math.round(_sourceNum)
                }
                if (_nowSub != null) {
                    ...
                    sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = _nowSub.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听说"), problemId = _nowSub.id, englishText = _nowSub.englishText, problemIndex = nowIndex.value.toString(10), isPass = if (_sourceNum >= meetDb.value) {
                        "1"
                    } else {
                        "0"
                    }
                    , isExercise = "1", didDo = true, selectOpt = "", totalScore = _sourceNum, userSpeakUrl = data.path))
                }
                if (_subList != null) {
                    if (nowIndex.value + 1 >= _subList.length) {
                        return
            ...
            val subStatus = computed(fun(): String {
                return getStatus(nowIndex.value)
            }
            )
                ...
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/studyTask/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听说"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = "", options = item.options ?: _uA()))
                        }
                        )
                    }
                }
            ...
            fun gen_getSubjectList_fn() {
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        , 1000)
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听说"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = ""))
                        }
                        )
                    }
                }
                ...
                uni_request<Result<UTSJSONObject>>(RequestOptions(url = getUrl("/biz/problem/api/getExerciseRecordDetail"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.getArray("appProblemList")!!.forEach(fun(item, index){
                            ...
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = (item as UTSJSONObject).getNumber("textbookUnitId") ?: 0, studyTaskId = 0, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能听写"), problemId = (item as UTSJSONObject).getNumber("id") ?: 0, englishText = (item as UTSJSONObject).getString("englishText") ?: "0", problemIndex = index.toString(10), isPass = (item as UTSJSONObject).getString("rightStatus") ?: "0", isExercise = "1", didDo = true, selectOpt = (item as UTSJSONObject).getString("selectOpt") ?: ""))
                        }
                        )
                                    ...
                                    if (isTrue(!unref(isShowResult))) {
                                        _cE("view", _uM("key" to 0, "style" to _nS(_uM("flex" to "1"))), _uA(
                                            ...
                                            _cE("view", _uM("class" to "word_box"), _uA(
                                                _cE("text", _uM("class" to "word_text"), _tD(unref(nowExplainText)), 1),
                                                _cE("text", _uM("class" to "word_tip"), _tD(unref(nowExplainType)), 1)
                                            )),
                                            _cE("view", _uM("style" to _nS(_uM("align-items" to "center", "margin-bottom" to "10rpx"))), _uA(
                                                _cV(_component_Recognition, _uM("onSuccess" to recognitionSuccess, "language" to unref(language), "category" to unref(category), "subject" to unref(nowExplain), "intIsShowStat" to false, "seconds" to 5), null, 8, _uA(
                                                    "language",
                                                    "category",
                                                    "subject"
                                                ))
                                    ...
                                    if (isTrue(unref(isShowResult))) {
                                        _cE("view", _uM("key" to 1, "class" to "adopt_box"), _uA(
                                            ...
                                            _cE("view", _uM("class" to "word_box"), _uA(
                                                _cE("text", _uM("class" to "word_text"), _tD(unref(nowExplainText)), 1),
                                                _cE("text", _uM("class" to "word_tip"), _tD(unref(nowExplainType)), 1),
                                                _cE("text", _uM("class" to "word_text", "style" to _nS(_uM("margin-top" to "10rpx", "font-size" to "20rpx"))), _tD(unref(nowSub)?.englishText), 5)
                                            )),
                                            _cE("view", _uM("style" to _nS(_uM("align-items" to "center", "margin-bottom" to "10rpx"))), _uA(
                                                _cE("text", _uM("class" to "adopt_status"), _uA(
                                                    _tD(unref(subStatus)) + " ",
                                                    if (isTrue(unref(nowSub)?.totalScore)) {
                                                        _cE("text", _uM("key" to 0, "class" to "adopt_status"), "(" + _tD(unref(nowSub)?.totalScore) + ")", 1)
                                                    } else {
                                                        _cC("v-if", true)
                                                    }
                                                )),
                                                if (isTrue(unref(nowIndexSpeakUrl))) {
                                                    _cE("view", _uM("key" to 0, "style" to _nS(_uM("display" to "flex", "flex-direction" to "row", "width" to "200rpx", "justify-content" to "space-around"))), _uA(
                                                        _cV(_component_u_playMp3, _uM("src" to unref(nowSub)?.soundUrl), null, 8, _uA(
                                                            "src"
                                                        )),
                                                        _cV(_component_u_playPcm, _uM("src" to unref(nowIndexSpeakUrl)), null, 8, _uA(
                                                            "src"
                                                        ))
                                                    ), 4)
                                                } else {
