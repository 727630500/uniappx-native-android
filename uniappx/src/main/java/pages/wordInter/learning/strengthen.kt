...
open class GenPagesWordInterLearningStrengthen : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesWordInterLearningStrengthen) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesWordInterLearningStrengthen
            ...
            fun gen_saveTempFn_fn(_index: Number) {
                ...
                if (uni_getStorageSync("mapKey") == "0") {
                    if (_index < 0) {
                ...
                if (uni_getStorageSync("mapKey") == null) {
                    if (_data.length == 0) {
                        uni_removeStorageSync("isBack")
                        uni_navigateBack(null)
                        return
            ...
            val inputClick = fun(){
                if (needShowKeyboard.value) {
                    ...
                    styleTransformWithOrigin.value?.style?.setProperty("transform", "translateY(0px)")
                }
            }
            fun gen_nextItem_fn() {
                            ...
                            _cE("view", _uM("id" to "styleTransformWithOrigin"), _uA(
                                ...
                                _cE("view", _uM("class" to "content_box"), _uA(
                                        ...
                                        _cE("view", _uM("style" to _nS(_uM("flex" to "1", "height" to "100%", "position" to "relative", "justify-content" to "center"))), _uA(
                                            if (unref(inspectStatus) != "fail") {
                                                _cE("view", _uM("key" to 0, "style" to _nS(_uM("flex" to "1", "justify-content" to "center", "padding" to "6rpx 10rpx 0"))), _uA(
                                                    _cV(_component_wj_input, _uM("id" to "helloView", "style" to _nS(_uM("width" to "100%", "height" to "32rpx")), "text" to unref(inputText), "onUpdate:text" to fun(`$event`: String){
                                    ...
                                    _cE("view", _uM("class" to "grammar_box"), _uA(
                                        ...
                                        ,
                                        _cV(_component_u_playMp3, _uM("src" to unref(nowItem)?.soundUrl, "isAuto" to true, "tplType" to 5, "ref_key" to "refMp3", "ref" to refMp3), null, 8, _uA(
                                            "src"
                                        )),
                                        _cE("text", _uM("id" to "text", "class" to "grammar_item"), _tD(unref(nowItem)?.chineseExplain), 1)
                                    ))
                                ))
                            ))
                        )),
