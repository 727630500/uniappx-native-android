...
open class GenPagesWordInterSilentlySilently : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesWordInterSilentlySilently) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesWordInterSilentlySilently
            val _cache = __ins.renderCache
            val subjectIteratorFn = subjectIterator()
            subjectIteratorFn.module = getModelKey("同步单词")
            subjectIteratorFn.subModule = getSubModelKey("单词智能默写")
            val ctx = createInnerAudioContext()
            ctx.src = getSondUrl("学习默写时输错警示音")
            val ctxPlayNow = createInnerAudioContext()
            val ctxPlayEnd = createInnerAudioContext()
            ctxPlayEnd.src = getSondUrl("学习答题通过提示音")
            ...
            fun gen_sendUp_fn(isPass: String) {
                ...
                var _data: UTSJSONObject = _uO("textbookId" to getTextBookId(), "textbookUnitId" to nowItem.value?.textbookUnitId, "module" to getModelKey("同步单词"), "subModule" to getSubModelKey("单词智能默写"), "problemId" to nowItem.value?.id, "englishText" to nowItem.value?.englishText, "problemIndex" to subjectIteratorFn.getIndex(nowItem.value), "isPass" to isPass, "isExercise" to "0", "studyTaskId" to studyTaskId.value)
            ...
            fun gen_goTest_fn() {
                if (isReview.value) {
                    uni_redirectTo(RedirectToOptions(url = "/pages/wordInter/silently/test?isReview=1"))
                    return
                }
                if (!isTest.value) {
                ...
                if (studyTaskId.value != 0) {
                    uni_redirectTo(RedirectToOptions(url = "/pages/wordInter/silently/test?studyTaskId=" + studyTaskId.value))
                } else {
                    uni_redirectTo(RedirectToOptions(url = "/pages/wordInter/silently/test"))
                }
            }
            ...
            fun gen_getReviewList_fn() {
                ...
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/getReviewEnglishProblemList"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        subjectIteratorFn.putArr(_data).then(fun(){
                            subjectIteratorFn.nextTempCount = getStrengthen("单词智能默写")
                            nextItem()
                        }
                        )
                                    ...
                                    _cE("view", _uM("class" to "word_inp_box"), _uA(
                                        _cE("view", _uM("style" to _nS(_uM("flex" to "1", "height" to "100%", "position" to "relative", "justify-content" to "center"))), _uA(
                                            if (unref(inspectStatus) != "fail") {
                                                _cV(_component_wj_input, _uM("key" to 0, "style" to _nS(_uM("width" to "100%", "height" to "32rpx")), "class" to "native-button", "text" to unref(inputText), "onUpdate:text" to fun(`$event`: String){
                                                    trySetRefValue(inputText, `$event`)
                                                }, "onInput" to onInput, "onInputTap" to inputClick, "checkText" to unref(nowItem)?.englishText, "isCheck" to (unref(inspectStatus) == "secondary"), "onOnSuccess" to fun(){
                                                    showFail.value = false
                                                    showCorrect.value = true
                                                }, "onOnErr" to fun(){
                                                    showFail.value = true
