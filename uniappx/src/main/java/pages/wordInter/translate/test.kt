...
open class GenPagesWordInterTranslateTest : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesWordInterTranslateTest) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesWordInterTranslateTest
            ...
            val nowExplain = computed(fun(): String {
                ...
                return if (nowIndex.value <= _len / 2) {
                    nowSub.value?.chineseExplain ?: ""
                } else {
                    nowSub.value?.englishText ?: ""
                }
            }
            )
            ...
            fun gen_submit_fn() {
                ...
                , "module" to getModelKey("同步单词"), "subModule" to getSubModelKey("单词智能翻译"), "score" to scoreVal.value, "startTime" to startTime.value, "passStatus" to passStatus, "second" to secondVal.value, "studyTaskId" to studyTaskId.value, "type" to if (studyTaskId.value != null) {
                    "10"
                } else {
                    "0"
                }
                ...
                if (_nowItem != null) {
                    ...
                    if (_options != null) {
                        ...
                        if (_nowSub != null) {
                            ...
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = _nowItem.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能翻译"), problemId = _nowSub.id, englishText = _nowSub.englishText, problemIndex = nowIndex.value.toString(10), isPass = if (_options[kVal].isRight) {
                                "1"
                            } else {
                                "0"
                            }
                            , isExercise = "1", didDo = true, selectOpt = _options[kVal].option, options = _nowItem.options))
                            ctx.play()
                        }
                        var _subList = subList.value
                        if (_subList != null) {
            ...
            fun gen_test_fn() {
                ...
                if (_nowSub != null) {
                    if (!successList.value.includes(_nowSub.id)) {
                        successList.value.push(_nowSub.id)
                        sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = _nowSub.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能翻译"), problemId = _nowSub.id, englishText = _nowSub.englishText, problemIndex = nowIndex.value.toString(10), isPass = "1", isExercise = "1", didDo = true, selectOpt = "", options = _nowSub.options))
                    }
                }
                var _subList = subList.value
                if (_subList != null) {
                ...
                uni_request<Result<UTSArray<followAlongItem>>>(RequestOptions(url = getUrl("/biz/problem/api/studyTask/list"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.forEach(fun(item, index){
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = item.textbookUnitId, studyTaskId = studyTaskId.value, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能翻译"), problemId = item.id, englishText = item.englishText, problemIndex = index.toString(10), isPass = "0", isExercise = "1", didDo = false, selectOpt = "", options = item.options ?: _uA()))
                        }
                        )
                    }
                }
                ...
                uni_request<Result<UTSJSONObject>>(RequestOptions(url = getUrl("/biz/problem/api/getExerciseRecordDetail"), method = "GET", data = object : UTSJSONObject() {
                    ...
                    if (_data != null) {
                        ...
                        _data.getArray("appProblemList")!!.forEach(fun(item, index){
                            ...
                            sendUpClassFn.push(sendUpType(textbookId = getTextBookId(), textbookUnitId = (item as UTSJSONObject).getNumber("textbookUnitId") ?: 0, studyTaskId = 0, module = getModelKey("同步单词"), subModule = getSubModelKey("单词智能翻译"), problemId = (item as UTSJSONObject).getNumber("id") ?: 0, englishText = (item as UTSJSONObject).getString("englishText") ?: "0", problemIndex = index.toString(10), isPass = (item as UTSJSONObject).getString("rightStatus") ?: "0", isExercise = "1", didDo = true, selectOpt = (item as UTSJSONObject).getString("selectOpt") ?: ""))
                        }
                        )
